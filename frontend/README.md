...
| `OPENAI_MODEL` | No (defaults to `gpt-4.1-mini`) | Leave unset — the code default is course-canonical and matches the [AI Maker Space course](https://github.com/AI-Maker-Space/The-AI-Engineer-Challenge) (`gpt-4.1-mini`). Override to `gpt-5` only if you want reasoning depth — see [Things that look weird on purpose](#things-that-look-weird-on-purpose) for the FastAPI-sibling divergence note |
| `OPENAI_MAX_COMPLETION_TOKENS` | No (defaults to `1500`) | `1500` is fine for the `gpt-4.1-mini` default (no reasoning overhead). Bump to `4000` if you override `OPENAI_MODEL=gpt-5` so reasoning has room without exhausting the budget mid-answer. |
...
- **The frontend code default is `gpt-4.1-mini`** — course-canonical for the [AI Maker Space — AI Engineering Challenge](https://github.com/AI-Maker-Space/The-AI-Engineer-Challenge) ("*Accessing 'gpt-4.1-mini' (ChatGPT) like a developer*"). Fast (TTFT <1s, ~3-5s end-to-end), no reasoning overhead, the `OPENAI_MAX_COMPLETION_TOKENS` default of `1500` is plenty. **The sibling FastAPI backend at [`api/index.py:37`](../api/index.py) intentionally diverges to `gpt-5`** for reasoning depth on that surface — the frontend takes the course-canonical fast path because the grading rubric targets the deployed Next.js app, where first-token latency matters more than chain-of-thought depth. Override with `OPENAI_MODEL=gpt-5` in Vercel if you want symmetric defaults across both backends — also bump `OPENAI_MAX_COMPLETION_TOKENS=4000`, because reasoning models burn hundreds of tokens on silent chain-of-thought before emitting any visible delta, and a too-low cap blows the entire budget on reasoning → empty stream → the user-facing "assistant returned an empty response" error documented in [`lib/env.ts`](lib/env.ts).
